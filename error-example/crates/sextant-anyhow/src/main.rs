...
use sextant_anyhow::{analyze_chart, analyzer::analyze_charts, report::ReportFormat};
...
async fn main() {
    ...
    if let Err(error) = result {
        // Use Debug format to include backtrace information
        eprintln!("Error: {:?}", error);

        // Note: async_backtrace::taskdump_tree() can be used to debug suspended async tasks
        // but is only useful when called while tasks are still running/suspended, not after
        // they've completed with errors. The #[async_backtrace::framed] annotations on async
        // functions help track logical call chains during task suspension.
...
async fn analyze_multiple_charts(
    ...
    if let Some(output_dir) = output_dir {
        ...
        if generate_summary {
            let summary_markdown = sextant_anyhow::report::generate_markdown_summary(&analyses);
...
fn print_chart_summary(analysis: &sextant_anyhow::ChartAnalysis) {
    eprintln!();
...
fn print_charts_summary(analyses: &[sextant_anyhow::ChartAnalysis]) {
    eprintln!();
