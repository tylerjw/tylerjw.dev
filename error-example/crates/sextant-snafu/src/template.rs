...
pub enum TemplateError {
    #[snafu(display("Failed to read template file {}: {}", path.display(), source))]
    TemplateFileRead {
        path: PathBuf,
        source: std::io::Error,
    },
    ...
    #[snafu(display("Failed to read values file {}: {}", path.display(), source))]
    ValuesFileRead {
        path: PathBuf,
        source: std::io::Error,
    },

    #[snafu(display("Failed to parse values file {}: {}", path.display(), source))]
    ValuesFileParse {
        path: PathBuf,
        source: serde_yaml::Error,
    },

    #[snafu(display("Templates directory does not exist: {}", path.display()))]
    TemplatesDirectoryNotFound {
        path: PathBuf,
        backtrace: snafu::Backtrace,
    },

    #[snafu(display("Failed to read templates directory {}: {}", path.display(), source))]
    TemplatesDirectoryRead {
        path: PathBuf,
        source: std::io::Error,
    },

    #[snafu(display("Failed to read chart directory {}: {}", path.display(), source))]
    ChartDirectoryRead {
        path: PathBuf,
        source: std::io::Error,
    },
    ...
    #[snafu(display("YAML serialization error: {source}"), context(false))]
...
impl Template {
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self, TemplateError> {
        ...
        let content = std::fs::read_to_string(path).with_context(|_| TemplateFileReadSnafu {
...
impl Values {
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self, TemplateError> {
        let path = path.as_ref();
        let content = std::fs::read_to_string(path).with_context(|_| ValuesFileReadSnafu {
            path: path.to_path_buf(),
        })?;

        let data: Value =
            serde_yaml::from_str(&content).with_context(|_| ValuesFileParseSnafu {
                path: path.to_path_buf(),
            })?;
...
pub fn find_template_files<P: AsRef<Path>>(
    ...
    if !templates_dir.exists() {
        return TemplatesDirectoryNotFoundSnafu {
    ...
    for entry in std::fs::read_dir(templates_dir).with_context(|_| TemplatesDirectoryReadSnafu {
        path: templates_dir.to_path_buf(),
    })? {
        let entry = entry?;
        let path = entry.path();
...
pub fn find_values_files<P: AsRef<Path>>(chart_dir: P) -> Result<Vec<PathBuf>, TemplateError> {
    ...
    for entry in std::fs::read_dir(chart_dir).with_context(|_| ChartDirectoryReadSnafu {
