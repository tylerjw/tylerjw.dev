...
pub enum AnalysisError {
    #[snafu(display("Chart analysis failed for {}: {}", path.display(), source))]
    ChartAnalysisFailed {
        path: PathBuf,
        source: crate::chart::ChartError,
    },

    #[snafu(display("Failed to load chart metadata: {source}"))]
    MetadataLoad { source: crate::chart::ChartError },

    #[snafu(display("Chart metadata validation failed: {message}"))]
    MetadataValidation {
        message: String,
        backtrace: snafu::Backtrace,
    },

    #[snafu(display("Failed to find templates in {}: {}", path.display(), source))]
    TemplatesNotFound {
        path: PathBuf,
        source: crate::template::TemplateError,
    },

    #[snafu(display("Failed to load template {}: {}", path.display(), source))]
    TemplateLoad {
        path: PathBuf,
        source: crate::template::TemplateError,
    },

    #[snafu(display("Failed to find values files in {}: {}", path.display(), source))]
    ValuesFilesNotFound {
        path: PathBuf,
        source: crate::template::TemplateError,
    },

    #[snafu(display("Failed to load values file {}: {}", path.display(), source))]
    ValuesFileLoad {
        path: PathBuf,
        source: crate::template::TemplateError,
    },

    #[snafu(display("Analysis failed for values file {name}: {source}"))]
    ...
    #[snafu(display("Analysis failed with empty values: {source}"))]
    EmptyValuesAnalysis {
        source: Box<AnalysisError>,
        backtrace: snafu::Backtrace,
    },

    #[snafu(display("Failed to render template {}: {}", path.display(), source))]
    TemplateRender {
        path: PathBuf,
        source: crate::template::TemplateError,
    },

    #[snafu(display("Failed to extract resources from template {}: {}", path.display(), source))]
    ResourceExtraction {
        path: PathBuf,
        source: Box<AnalysisError>,
        backtrace: snafu::Backtrace,
    ...
    #[snafu(display("Charts directory does not exist: {}", path.display()))]
    ChartsDirectoryNotFound {
        path: PathBuf,
        backtrace: snafu::Backtrace,
    },

    #[snafu(display("Failed to read charts directory {}: {}", path.display(), source))]
    ChartsDirectoryRead {
        path: PathBuf,
        source: std::io::Error,
    },

    #[snafu(display("Invalid YAML resource in template {}: {}", path.display(), source))]
    InvalidYamlResource {
        path: PathBuf,
    ...
    #[snafu(display("YAML parsing error: {source}"), context(false))]
...
pub fn analyze_chart<P: AsRef<Path>>(chart_dir: P) -> Result<ChartAnalysis, AnalysisError> {
    ...
    let chart_file = find_chart_file(chart_dir).with_context(|_| ChartAnalysisFailedSnafu {
        path: chart_dir.to_path_buf(),
    })?;

    let chart_metadata = ChartMetadata::load_from_file(&chart_file).context(MetadataLoadSnafu)?;

    chart_metadata
        .validate()
        .map_err(|e| AnalysisError::MetadataValidation {
            message: format!("{}", e),
    ...
    let template_files = if templates_dir.exists() {
        find_template_files(&templates_dir).with_context(|_| TemplatesNotFoundSnafu {
    ...
    for template_path in template_files {
        let template =
            Template::load_from_file(&template_path).with_context(|_| TemplateLoadSnafu {
    ...
    // Find values files
    let values_files = find_values_files(chart_dir).with_context(|_| ValuesFilesNotFoundSnafu {
        path: chart_dir.to_path_buf(),
    })?;
    ...
    for values_path in values_files {
        ...
        let values =
            Values::load_from_file(&values_path).with_context(|_| ValuesFileLoadSnafu {
    ...
    if analysis.values_file_count() == 0 {
        ...
        let resource_report = analyze_with_values(&templates, &empty_values).map_err(|source| {
            AnalysisError::EmptyValuesAnalysis {
                source: Box::new(source),
                backtrace: snafu::Backtrace::capture(),
            }
        })?;
...
fn analyze_with_values(
    ...
    for template in templates {
        let rendered = template
            .render(values)
            .with_context(|_| TemplateRenderSnafu {
...
pub async fn analyze_charts<P: AsRef<Path>>(
    ...
    if !charts_dir.exists() {
        return ChartsDirectoryNotFoundSnafu {
    ...
    for entry in std::fs::read_dir(charts_dir).with_context(|_| ChartsDirectoryReadSnafu {
        path: charts_dir.to_path_buf(),
    })? {
        let entry = entry?;
        let path = entry.path();
