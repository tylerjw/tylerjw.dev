...
pub enum ReportError {
    #[snafu(
        display("Failed to serialize chart analysis to JSON: {source}"),
        context(false)
    )]
    JsonSerialization { source: serde_json::Error },

    #[snafu(
        display("Failed to serialize chart analysis to YAML: {source}"),
        context(false)
    )]
...
impl ChartAnalysis {
    ...
    pub fn save_to_file<P: AsRef<Path>>(
        ...
        std::fs::write(path, content).with_context(|_| FileWriteSnafu {
...
mod tests {
    ...
    fn test_create_chart_analysis() {
        ...
        assert!(analysis.metadata.has_dependencies());
