...
use sextant_snafu::{
    Error, Result, analyze_chart, analyzer::analyze_charts, report, report::ReportFormat,
};
...
async fn main() {
    ...
    if let Err(error) = result {
        ...
        // Print backtrace if available and not disabled by RUST_BACKTRACE=0
        if let Some(backtrace) = ErrorCompat::backtrace(&error) {
            if env::var("RUST_BACKTRACE").unwrap_or_default() != "0" {
                eprintln!("\nBacktrace:\n{}", backtrace);
            }
        }
...
async fn analyze_multiple_charts(
    ...
    if let Some(output_dir) = output_dir {
        ...
        if generate_summary {
            let summary_markdown = report::generate_markdown_summary(&analyses);
            let summary_path = output_dir.join("summary.md");

            std::fs::write(&summary_path, summary_markdown)?;
    ...
    print_multiple_charts_summary(&analyses);

    Ok(())
}

fn parse_format(format: &str) -> Result<ReportFormat> {
    ReportFormat::from_extension(format).ok_or_else(|| Error::Custom {
        message: format!(
...
fn print_chart_summary(analysis: &sextant_snafu::ChartAnalysis) {
    eprintln!();
    ...
    eprintln!(
        "Dependencies: {}",
        if analysis.metadata.has_dependencies() {
            analysis.metadata.dependencies.as_ref().unwrap().len()
        } else {
            0
        }
    );
...
fn print_multiple_charts_summary(analyses: &[sextant_snafu::ChartAnalysis]) {
    eprintln!();
