...
impl ChartMetadata {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.name.is_empty() {
            color_eyre::eyre::bail!("Chart name cannot be empty");
        }

        if self.version.is_empty() {
            color_eyre::eyre::bail!("Chart version cannot be empty");
        }
...
pub fn find_chart_file<P: AsRef<Path>>(chart_dir: P) -> Result<std::path::PathBuf> {
    ...
    color_eyre::eyre::bail!(
        "No Chart.yaml or Chart.yml found in {}",
        chart_dir.display()
    );
}
