...
pub enum TemplateError {
    ...
    #[error("Failed to read values file {}: {source}", .path.display())]
    ValuesFileRead {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    ...
    #[error("Failed to parse values file {}: {source}", .path.display())]
    ValuesFileParse {
        path: PathBuf,
        #[source]
    ...
    TemplatesDirectoryNotFound { path: PathBuf },

    #[error("Failed to read templates directory {}: {source}", .path.display())]
    TemplatesDirectoryRead {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    },

    #[error("Failed to read chart directory {}: {source}", .path.display())]
    ChartDirectoryRead {
        path: PathBuf,
        #[source]
        source: std::io::Error,
...
impl Template {
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self, TemplateError> {
        ...
        let content =
            std::fs::read_to_string(path).map_err(|source| TemplateError::TemplateFileRead {
    ...
    pub fn render(&self, values: &Values) -> Result<RenderedTemplate, TemplateError> {
        ...
        rendered = self
            ...
            .map_err(|source| TemplateError::RenderFailed {
    ...
    fn substitute_variables(&self, content: &str, values: &Value) -> Result<String, TemplateError> {
    ...
    fn handle_conditionals(&self, content: &str, _values: &Value) -> Result<String, TemplateError> {
...
impl Values {
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self, TemplateError> {
        let path = path.as_ref();
        let contents =
            std::fs::read_to_string(path).map_err(|source| TemplateError::ValuesFileRead {
                path: path.to_path_buf(),
                source,
            })?;

        let data: Value =
            serde_yaml::from_str(&contents).map_err(|source| TemplateError::ValuesFileParse {
                path: path.to_path_buf(),
                source,
            })?;
    ...
    pub fn merge(&self, other: &Values) -> Result<Values, TemplateError> {
...
pub fn find_template_files<P: AsRef<Path>>(
    templates_dir: P,
    ...
    for entry in std::fs::read_dir(templates_dir).map_err(|source| {
        TemplateError::TemplatesDirectoryRead {
            path: templates_dir.to_path_buf(),
            source,
        }
    })? {
...
pub fn find_values_files<P: AsRef<Path>>(chart_dir: P) -> Result<Vec<PathBuf>, TemplateError> {
    ...
    for entry in
        std::fs::read_dir(chart_dir).map_err(|source| TemplateError::ChartDirectoryRead {
