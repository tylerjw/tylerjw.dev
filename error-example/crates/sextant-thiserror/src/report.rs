...
pub enum ReportError {
    #[error("Failed to serialize chart analysis to JSON: {0}")]
    JsonSerialization(#[from] serde_json::Error),

    #[error("Failed to serialize chart analysis to YAML: {0}")]
...
impl ChartAnalysis {
    ...
    pub fn save_to_file<P: AsRef<Path>>(
        ...
        std::fs::write(path, content).map_err(|source| ReportError::FileWrite {
...
mod tests {
    ...
    #[test]
    fn test_chart_analysis_with_reports() {
        let metadata = create_test_chart_metadata();
    ...
    #[test]
    fn test_generate_markdown_summary() {
        let metadata = create_test_chart_metadata();
        let mut analysis = ChartAnalysis::new(
            "test-chart".to_string(),
            "1.0.0".to_string(),
