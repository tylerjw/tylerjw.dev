...
use sextant_thiserror::{
    Error, Result, analyze_chart, analyzer::analyze_charts, report::ReportFormat,
};
...
async fn main() {
    ...
    if let Err(error) = result {
        ...
        // Print backtrace if available and enabled (using async-backtrace)
...
async fn analyze_multiple_charts(
    ...
    if let Some(output_dir) = output_dir {
        ...
        if generate_summary {
            let summary_markdown = sextant_thiserror::report::generate_markdown_summary(&analyses);
...
fn print_chart_summary(analysis: &sextant_thiserror::ChartAnalysis) {
    eprintln!();
...
fn print_charts_summary(analyses: &[sextant_thiserror::ChartAnalysis]) {
    eprintln!();
